...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dias        = $_POST['dias'] ?? [];
    $dias_csv    = sanitizar_dias_atencion_csv($dias);
    ...
    foreach ($offs as $d) {
        ...
        $fechaOff = sprintf('%04d-%02d-%02d', $anio, $mes, $dia);
        if (es_dia_cerrado_fijo($fechaOff)) {
            continue;
        }
        $insOff->execute([$fechaOff]);
    }

    db()->exec('DELETE FROM meses_visibles');
    $insMes = db()->prepare('INSERT INTO meses_visibles (anio, mes) VALUES (?, ?)');
    $vistos = $_POST['meses_visibles'] ?? [];
    $vistos[] = sprintf('%04d-%02d', (int) date('Y'), (int) date('n'));
    $ya = [];
    foreach ($vistos as $ym) {
    ...
    ir(url('admin/disponibilidad.php?anio=' . $anio . '&mes=' . $mes));
}
...
$mes_hoy_key = sprintf('%04d-%02d', (int) date('Y'), (int) date('n'));
?>
...
<p class="avail-sub">Marca días libres, define el horario y elige qué meses siguientes pueden ver las clientas. El mes actual se marca solo.</p>
</div>
</header>
...
<?php else:
    ...
    $cerrado = es_dia_cerrado_fijo($fechaDia);
    $checked = $cerrado || in_array($dia, $offs, true);
?>
<label class="avail-day<?php echo $cerrado ? ' is-cerrado' : ''; ?>">
<?php if ($cerrado): ?>
...
<p class="hint">Lunes y domingo quedan siempre ocupados. El resto está disponible; toca un día para marcarlo como no disponible. Solo se guarda este mes.</p>
</article>
...
<p class="hint">De martes a viernes vale este horario. El sábado se atiende hasta las <strong>2:30 p.m.</strong>.</p>
<p class="avail-label">Días de atención</p>
<div class="chip-grid">
<?php foreach ($DIAS_ES as $n => $nombre):
    $cerrado = ($n === 1 || $n === 7);
?>
<label class="chip<?php echo $cerrado ? ' is-cerrado' : ''; ?>">
<?php if ($cerrado): ?>
<input type="checkbox" value="<?php echo $n; ?>" disabled>
<?php else: ?>
...
<p class="hint">Lunes y domingo están siempre ocupados. Martes a sábado quedan disponibles por defecto.</p>
</article>

<article class="avail-card">
<h3>Meses visibles</h3>
<p class="hint">El mes actual siempre está visible para la clienta. Los meses anteriores no se muestran al reservar. Marca solo los siguientes que quieras abrir.</p>
...
<?php for ($i = 1; $i <= 12; $i++):
    ...
    $es_actual = $key === $mes_hoy_key;
    $chk = $es_actual || isset($visibles_map[$key]);
?>
<label class="vis-month<?php echo $es_actual ? ' is-now' : ''; ?>">
<?php if ($es_actual): ?>
<input type="hidden" name="meses_visibles[]" value="<?php echo h($key); ?>">
<input type="checkbox" value="<?php echo h($key); ?>" checked disabled>
<?php else: ?>
...
<span class="vis-name"><?php echo h($MESES_ES[$i]); ?><?php echo $es_actual ? ' · actual' : ''; ?></span>
</label>
<?php endfor; ?>
</div>
<?php endforeach; ?>
...
window.HHS_MES_ACTUAL = <?php echo json_encode($mes_hoy_key); ?>;
(function () {
  ...
  function panelDeAnio(year) {
    ...
    (window.HHS_MESES || []).forEach(function (nombre, i) {
      ...
      var key = year + '-' + mm;
      var locked = key === window.HHS_MES_ACTUAL;
      var lab = document.createElement('label');
      lab.className = 'vis-month' + (locked ? ' is-now' : '');
      lab.innerHTML = (locked ? '<input type="hidden" name="meses_visibles[]" value="' + key + '"><input type="checkbox" value="' + key + '" checked disabled>' : '<input type="checkbox" name="meses_visibles[]" value="' + key + '">') +
        '<span class="vis-check" aria-hidden="true"></span>' +
        '<span class="vis-name"></span>';
      lab.querySelector('.vis-name').textContent = nombre + (locked ? ' · actual' : '');
  ...
  function elegir(menu, year) {
    ...
    if (menu.getAttribute('data-nav') === '1') {
      ...
      var dest = <?php echo json_encode(url('admin/disponibilidad.php')); ?> + '?anio=' + encodeURIComponent(year) + '&mes=' + encodeURIComponent(m);
      if (window.navigateApp) {
        window.navigateApp(dest, true);
      } else {
  ...
  if (mesSel) {
    mesSel.addEventListener('change', function () {
      ...
      if (!y) return;
      var dest = <?php echo json_encode(url('admin/disponibilidad.php')); ?> + '?anio=' + encodeURIComponent(y) + '&mes=' + encodeURIComponent(mesSel.value);
      if (window.navigateApp) {
        window.navigateApp(dest, true);
      } else {
        window.location.replace(dest);
